...
class Git {
    ...
    analyzerBranch(branch) {
         ...
         *
         * model: 分支模型（master | test | release | hotfix | copy-release | copy-hotfix）
         * project: mri 项目名称
    ...
    getProjectVersionWithBranch(project) {
        ...
        let tags = _.map(branches, (branch) => {
            branch = _.trim(branch);
            let tag = branch.replace(new RegExp(`^.*/${project}/v(.*)$`, 'g'), '$1');
            tag = tag || '0.0.1';
            return tag;
        });
