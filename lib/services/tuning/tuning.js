...
class Tuning {
     ...
     */
    pkg() {
        let pkgPath = _join(process.cwd(), './package.json.ejs');
        let pkg = $util.loadJSON(pkgPath) || {};
        let source = $util.loadJSON(_join(__dirname, './template/package.json.ejs')) || {};

        pkg = _.extend({}, source, pkg);

        let config = $util.loadjs(_join(process.cwd(), './.mrirc.js')) || {};
